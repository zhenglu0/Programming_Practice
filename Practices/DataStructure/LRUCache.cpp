...
/*
    ...
    1. A Queue which is implemented using a doubly linked list. The maximum
    ...
    update the corresponding node address in the hash. If the queue is full,
    i.e. all the frames are full, we remove a node from the rear of queue,
    and add the new node to the front of queue.
*/
//  This problem can also be used extend to the L1 L2 Cache and the Block Cache
//
//  For the L1 L2 or other hierarchical Cache, When data not in L1, we read L2
//  then load it to L1. If not in L2 We read it to L2
//
//  When we read not in Cache, we also read the data near the block, and load it into
//  Cache (ex. map application)
